...
class GulpPugSassTsStateGenerator {
	LinkedHashSet<State> states
	Util util = new Util()
	String fname
	LinkedHashMap<String, String> nsl
	GulpPugSassTsElementGenerator pugElementGenerator = new GulpPugSassTsElementGenerator()	
	...
	def void genStates(Node node, String fname, String path, IFileSystemAccess2 fsa, LinkedHashSet<State> states, LinkedHashMap<String, String> nsl) {
		...
		this.nsl = nsl
		this.states = states		
		fsa.generateFile("pug/views/"+path+"/" + fname + ".pug", this.genBaseState(State.getOverrideApp(node), states))				
		for(state : states){			
			fsa.generateFile("pug/views/"+path+"/" + fname + "_" + state.name.toFirstLower +".pug", this.genState(State.getOverrideApp(node), State.basedOnState(state, states)))
		}			
	}
