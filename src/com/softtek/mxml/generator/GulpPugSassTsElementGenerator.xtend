...
class GulpPugSassTsElementGenerator {
	Util util = new Util()
...
«««			«this.genDateChooser(indentation, n)»
	...
	«ELSEIF n.prefix.equals('MedListaCriteriosView')»
		«this.genMedListaCriteriosView(indentation, n)»
	«ELSEIF n.prefix.equals('MedRangoFechaView')»
		«this.genMedRangoFechaView(indentation, n)»
	«ELSEIF n.prefix.equals('MedDateFieldView')»
		«this.genMedDateFieldView(indentation, n)»
	«ELSEIF n.prefix.equals('MedListaDesplegableView')»
		«this.genMedListaDesplegableView(indentation, n)»
	...
	block styles
	   link(rel="stylesheet" href="../../assets/css/accimarket/«fname»/styles.css")
	   
	block scripts
	   script(src="../../assets/scripts/accimarket/«fname»/scripts.js")
	...
	def String genTextInput(int indentation, NodeOverride no)'''
		«var id = getProperty(no, "id")»
		«var flexOverride = getProperty(no, "flexOverride")»
		«IF flexOverride !== null && !flexOverride.equals("")»		
		«util.getIndentation(indentation)»div(id="«id»" class="«flexOverride»")
		«util.getIndentation(indentation+1)»+field-input("«id»",{label: "Example", required: true, orientation: "horizontal"})
		«ELSE»
		«util.getIndentation(indentation)»+field-input("«id»",{label: "Example", required: true, orientation: "horizontal"})
		«ENDIF»		
	'''
	...
	//Buttons
	def String getMixinButton(NodeOverride no){
		var label = getProperty(no, "label")
		var id = getProperty(no, "id") !== null ? getProperty(no, "id") : ""
		var mixin = ""
		if(label !== null && !label.equals("")){
			if(label.equalsIgnoreCase("Aceptar")){
				mixin = "button-accept"
			}else if(label.equalsIgnoreCase("Aceptar") || label.toLowerCase.contains("Aceptar") || id.toLowerCase.contains("add")){
				mixin = "button-add"
			}else if(label.equalsIgnoreCase("Limpiar") || id.toLowerCase.contains("limpiar")){
				mixin = "button-clean"
			}else if(label.equalsIgnoreCase("Cancelar") || label.toLowerCase.contains("Cancelar")){
				mixin = "button-cancel"
			}else if(id.toLowerCase.contains("remove")){
				mixin = "button-delete"
	...
	def String genButton(int indentation, NodeOverride no)'''
		«var id = getProperty(no, "id", "id")»
		«var flexOverride = getProperty(no, "flexOverride")»
		«var tooltip = getProperty(no, "id") !== null && !getProperty(no, "id").equals("") ? ", tooltip:\"" + getProperty(no, "id") + "\"" : ""»
		«IF flexOverride !== null && !flexOverride.equals("")»
		«util.getIndentation(indentation)»div(id="«id»" class="«flexOverride»")
		«util.getIndentation(indentation+1)»+«getMixinButton(no)»({size:"large", disabled: false«tooltip»})
		«ELSE»
		«util.getIndentation(indentation)»+«getMixinButton(no)»({size:"large", disabled: false«tooltip»})
		«ENDIF»		
	'''
	...
	def String genRadioButton(int i, NodeOverride no)'''
		«var id = getProperty(no, "id", "id")»
		«var value = getProperty(no, "value") !== null ? getProperty(no, "value") : "NoValue"»
		«var label = getProperty(no, "label") !== null ? getProperty(no, "label") : "NoLabel"»
		«var flexOverride = getProperty(no, "flexOverride")»
		«var indentation = flexOverride !== null && !flexOverride.equals("") ? i+1 :i»
		«IF flexOverride !== null && !flexOverride.equals("")»	
		«util.getIndentation(i)»div(id="«id»" class="«flexOverride»")
		«ELSE»
		«util.getIndentation(indentation)»+field-options("«id»", {label: "«id»",},
		«util.getIndentation(indentation)»        {key: "«value»", value:"«label»"}
		«util.getIndentation(indentation)»)
		«ENDIF»
	'''
	
	def String genCheckBoxButton(int i, NodeOverride no)'''
		«var id = getProperty(no, "id")»
		«var flexOverride = getProperty(no, "flexOverride")»
		«var indentation = flexOverride !== null  && !flexOverride.equals("") ? i +1 : i»
		«IF flexOverride !== null && !flexOverride.equals("")»
		«util.getIndentation(i)»div(id="«id»" class="«flexOverride»")
		«ENDIF»
		«util.getIndentation(indentation)»+field-checks("«id»", {label: "«id»",},
		«util.getIndentation(indentation)»              {key: "TEST1", value:"TEST1"}
		«util.getIndentation(indentation)»)
    '''
	...
	def String genDateField(int indentation, NodeOverride no)'''
		...
		«util.getIndentation(indentation)»div(id="«id»" class="«flexOverride»")
		«util.getIndentation(indentation+1)»+field-date("«id»", {label: "«id»", clear:true})
		«ELSE»
		«util.getIndentation(indentation)»+field-date("«id»", {label: "«id»", clear:true})
		«ENDIF»		
	'''
...
//	'''
	...
	def String genMedListaCriteriosView(int indentation, NodeOverride no)'''
		«var id = getProperty(no, "id")»
		«var flexOverride = getProperty(no, "flexOverride")»
		«IF flexOverride !== null && !flexOverride.equals("")»
		«util.getIndentation(indentation)»div(id="«id»" class="«flexOverride»")
		«util.getIndentation(indentation+1)»+field-plus-minus("«id»", {label: "«id»"})
		«ELSE»
		«util.getIndentation(indentation)»+field-plus-minus("«id»", {label: "«id»"})
		«ENDIF»
	'''
	
	def String genMedRangoFechaView(int indentation, NodeOverride no)'''
		«var id = getProperty(no, "id")»
		«var flexOverride = getProperty(no, "flexOverride")»
		«IF flexOverride !== null && !flexOverride.equals("")»
		«util.getIndentation(indentation)»div(id="«id»" class="«flexOverride»")
		«util.getIndentation(indentation+1)»+field-date-range("«id»", {begin_label: "Fecha inicial «id»", end_label: "Fecha final", required: true, size:"10em",  clear:true, orientation:"vertical"})
		«ELSE»
		«util.getIndentation(indentation)»+field-date-range("«id»", {begin_label: "Fecha inicial «id»", end_label: "Fecha final", required: true, size:"10em",  clear:true, orientation:"vertical"})
		«ENDIF»		
	'''
	
	def String genMedDateFieldView(int indentation, NodeOverride no)'''
		«var id = getProperty(no, "id")»
		«var flexOverride = getProperty(no, "flexOverride")»
		«IF flexOverride !== null && !flexOverride.equals("")»
		«util.getIndentation(indentation)»div(id="«id»" class="«flexOverride»")
	...
	def String genMedListaDesplegableView(int indentation, NodeOverride no)'''
		«var id = getProperty(no, "id")»
		«var flexOverride = getProperty(no, "flexOverride")»
		«IF flexOverride !== null && !flexOverride.equals("")»
		«util.getIndentation(indentation)»div(id="«id»" class="«flexOverride»")
		«util.getIndentation(indentation+1)»+field-select("«id»", «util.getIndentation(indentation)»   { label: "«id»", size: "12em" },
		«util.getIndentation(indentation+1)»            {key: "V01", value:"Value 01"},
		«util.getIndentation(indentation+1)»            {key:"V02", value:"Value 02"} )
		«ELSE»
		«util.getIndentation(indentation)»+field-select("«id»", «util.getIndentation(indentation)»   { label: "«id»", size: "12em" },
		«util.getIndentation(indentation)»            {key: "V01", value:"Value 01"},
		«util.getIndentation(indentation)»            {key:"V02", value:"Value 02"} )
		«ENDIF»		
	'''
	...
	def String genIncludeComponent(int indentation, NodeOverride no, LinkedHashMap<String, String> nsl){
		...
		if (nsl.get(no.prefix) !== null) {
			...
			if (nsl.get(no.prefix).contains("http:") || nsl.get(no.prefix).contains("https:")) comment="//-"{
				if(flexOverride !== null && !flexOverride.equals("")){
					util.getIndentation(indentation+1) + "div(" + flexOverride + ")"
					util.getIndentation(indentation+1) + comment + "include /" + nsl.get(no.prefix).replace("*", no.name)+".pug"
				}else{
