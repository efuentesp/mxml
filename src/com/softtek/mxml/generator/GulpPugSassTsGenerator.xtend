...
class GulpPugSassTsGenerator {
	...
	GulpPugSassTsStateGenerator pugStateGenerator = new GulpPugSassTsStateGenerator()
		...
		nsl =  new LinkedHashMap<String, String>()
      	...
      	if(!this.appStates.empty){
			if(this.appStates.containsKey(fname)){
				...
				fsa.generateFile("pug/views/"+path+"/"+util.removeNameDecorator(fname)+".pug", genNodes(-2, overrideNode) )
			}
		}else{
			fsa.generateFile("pug/views/"+path+"/"+util.removeNameDecorator(fname)+".pug", genNodes(-2, overrideNode))
		}		    	
